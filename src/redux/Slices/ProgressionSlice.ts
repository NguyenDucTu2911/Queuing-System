...
export const addProgressions = createAsyncThunk("Service/addService",async (newData: Progressions)=>{
    try{
        if(newData){
            const data = await db.collection("Progression")
                .where("Maid", "==", newData.STT).get()
            if(!data.empty){
