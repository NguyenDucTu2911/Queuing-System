...
//add Service
export const addService = createAsyncThunk("Service/addService",async (newData: Services)=>{
    try{
        if(newData){
            const data = await db.collection("Service")
                .where("Maid", "==", newData.Maid).get()
            if(!data.empty){
...
//Edit Service
export const UpdateService = createAsyncThunk("Service/UpdateService",async (item: Services)=>{
    try{
