...
import { Devices, addReport, fetchDevice } from '../../redux/Slices/deviceSlice';
...
const Device: React.FC<DeviceProps> = (props) => {
    ...
    //select kết nối
    const changeSelectConnect = (event: React.ChangeEvent<HTMLSelectElement>) => {
        const value = event.target.value;
        console.log(value)
        if (value === "Kết Nối" || value === "Mất Kết Nối") {
            const filteredData = devicessss.filter(
                (item) => item && item.Connect && item.Connect === value
            );
        ...
        <>
            <div className="devices">
                ...
                <div className="devices-action">
                    <label className='devices-lb' htmlFor="action">Trạng Thái Hoạt động</label>
